...
<head>
    ...
    <title>EDIT Bank of England</title>
    ...
    <div class="content-wrapper"><?php include('message.php'); ?>
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form action="code.php" method="POST" style="margin-bottom:16px">
                    <input type="hidden" name="bulk_edit_info" value="1"><input type="hidden" name="table" value="boe"><input type="hidden" name="redirect_path" value="boe_index.php">
                ...
                <form action="code.php" method="POST">
                    <input type="hidden" name="bulk_edit_balance_price" value="1"><input type="hidden" name="table" value="boe"><input type="hidden" name="redirect_path" value="boe_index.php">
        ...
        <div class="table-responsive" style="margin-top:16px">
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    <?php $query = "SELECT * FROM boe";
                    $query_run = mysqli_query($con, $query);
                    $hasEdit = file_exists('boe_edit.php');
                    $hasDel = file_exists('code_boe.php');
                    if ($query_run && mysqli_num_rows($query_run) > 0) {
                        foreach ($query_run as $row) { 
                            ...
                            <tr>
                                ...
                                <td><?php if ($hasEdit) { ?><a href="boe_edit.php?id=<?= $row['id']; ?>" class="btn btn-success btn-sm">Edit</a><?php } else { echo '—'; } ?>
                                    <?php if ($hasDel) { ?><form action="code_boe.php" method="POST" class="d-inline"><button type="submit" name="delete_student" value="<?= $row['id']; ?>" class="btn btn-danger btn-sm">Delete</button></form><?php } ?></td>
                            </tr>
                    <?php }
                    } else {
